        ...
        container('mspdotnet') {
          script {
            withCredentials([usernamePassword(credentialsId: 'nexus-credentials', passwordVariable: 'NEXUS_PASSWORD', usernameVariable: 'NEXUS_USERNAME')]) {
              def nexusUrl = "http://nexusrepo-sonatype-nexus-service.tools.svc.cluster.local:8081/repository/nuget-group"
...
dotnet --version
             ...
             # wget https://dist.nuget.org/win-x86-commandline/latest/nuget.exe -O /usr/local/bin/nuget
              #chmod +x /usr/local/bin/nuget
                ...
                dotnet nuget setapikey 7eb5424c-5f47-381c-b1fa-8c8592508455 -source http://nexusrepo-sonatype-nexus-service.tools.svc.cluster.local:8081/repository/nuget-group/appsettings.json/
