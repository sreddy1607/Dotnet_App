...
$SiteFolder = 'E:\inetpub'
$StagingFolder = 'E:\tar-etar-web-staging'
...
# Remove Existing and Copy Deployed Files for ETarWeb if app deployed
if (Test-Path $StagingFolder\ETarWeb\*) {
  Write-Host "Removing existing ETarWeb files from $SiteFolder\ETarWeb"
  Remove-Item -Recurse $SiteFolder\ETarWeb\*
  Write-Host "Removal status for ETarWeb files: $?"
  Write-Host "Copying newly deployed ETarWeb files to $SiteName\ETarWeb"
  xcopy /s/y/e  $StagingFolder\ETarWeb $SiteFolder\ETarWeb
  Write-Host "Copy status for ETarWeb files: $?"
}
